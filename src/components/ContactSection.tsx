  ...
  return (
    <section id="contact" className="py-16 lg:py-20">
      <div className="container mx-auto px-4 lg:px-8">
        ...
        >
          <h2 className="text-3xl lg:text-4xl font-bold mb-3">{t(translations.contact.title, lang)}</h2>
          <div className="h-1 w-16 gradient-line rounded-full mx-auto mb-3" />
          <p className="text-muted-foreground text-sm">{t(translations.contact.subtitle, lang)}</p>
        </motion.div>
        ...
        <div className="grid lg:grid-cols-2 gap-10">
          ...
          >
            {(["name", "email", "phone"] as const).map((field) => (
              <div key={field}>
                <label className="block text-xs font-medium mb-1">{t(translations.contact.form[field], lang)}</label>
                <input
            ...
            <div>
              <label className="block text-xs font-medium mb-1">{t(translations.contact.form.message, lang)}</label>
              <textarea
                required
                rows={3}
                value={form.message}
          ...
          >
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3 text-sm text-muted-foreground">
                ...
                <a href={`tel:${info.phone}`} className="hover:text-foreground transition-colors">
                  {info.phone}
                </a>
              </div>
              <div className="flex items-center gap-3 text-sm text-muted-foreground">
                <Mail className="w-4 h-4 text-primary flex-shrink-0" />
                <a href={`mailto:${info.email}`} className="hover:text-foreground transition-colors">
                  {info.email}
                </a>
              </div>
            </div>
